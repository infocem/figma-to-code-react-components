 ...
 *   FIGMA_TOKEN=figd_xxx node scripts/fetch-figma-cache.mjs --reset   # restart from scratch
 */
...
const CACHE_DIR = path.join(ROOT, 'apps', 'blue-hare-showcase', '.figma-cache');
...
const RETRY_DELAY_MS = 60000; // 1 minute wait on 429
const MAX_RETRIES = 3;
...
async function main() {
  ...
  console.log(`📦  Blue Hare DS — Figma Cache Fetcher`);
