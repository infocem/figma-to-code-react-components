...
3. **Document Extracted Specifications**
   ...
   **Icons and Assets:**
   - All `IMAGE-SVG` nodes and image fills must be downloaded in Phase 1b via `mcp__figma__download_figma_images` before writing any component code. See `references/asset-download-guide.md`.
   - Import icons from the `Icons.tsx` barrel generated in Phase 1b, unless the project has a confirmed icon library that contains the specific icon.
